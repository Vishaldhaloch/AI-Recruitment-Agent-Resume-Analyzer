...
class ResumeAnalysisAgent:
    ...
    def extract_text_from_pdf(self, pdf_file):
        ...
        try:
            ...
            return text[:8000]  # max 8k characters
    ...
    def create_vector_store(self, text):
        """Create simple vector store from text"""
        embeddings=HuggingFaceEmbeddings(model_name="sentence-transformers/paraphrase-MiniLM-L6-v2")
    ...
    def analyze_skill(self, qa_chain, skill):
        """Analyze individual skill proficiency"""
        ...
        try:
            ...
            return skill, 0, "Error"

    def analyze_resume_weaknesses(self):
        """Analyze weaknesses in the resume"""
        ...
        for skill in self.analysis_result.get("missing_skills", []):
            llm = ChatGroq(model="llama3-8b-8192", api_key=self.api_key)
    ...
    def semantic_skill_analysis(self, resume_text, skills):
        """Analyze skills semantically using vector similarity"""
    ...
    def analyze_resume(self, resume_file, role_requirements=None, custom_jd=None):
        """Main method to analyze a resume against role requirements or a custom JD"""
        self.resume_text = self.extract_text_from_file(resume_file)

        if not self.resume_text or len(self.resume_text) < 100:
            return {"error": "Resume text could not be extracted or is too short."}

        # Optional: Save to temp file for inspection/debugging
        ...
        # Extract skills from JD or use role requirements
        if custom_jd:
            ...
            return {"error": "No role requirements or JD provided."}

        # Analyze skills using semantic similarity
        self.analysis_result = self.semantic_skill_analysis(self.resume_text, self.extracted_skills)

        # Run weakness analysis only if there are missing skills
        if self.analysis_result.get("missing_skills"):
    ...
    # def analyze_resume(self, resume_file, role_requirements=None, custom_jd=None):
    #     """Main method to analyze a resume safely for lightweight usage"""
    #     self.resume_text = self.extract_text_from_file(resume_file)
    ...
    #     # Weakness analysis removed for speed/lightweight use
    #     # Run weakness analysis after semantic scoring
    #     # self.analyze_resume_weaknesses()
    #     return self.analysis_result
    ...
    def improve_resume(self, improvement_areas, target_role=""):
        ...
        try:
            ...
            # Handle 'Skills Highlighting' manually using weakness analysis
            ...
            # Handle remaining improvement areas with AI
            remaining_areas = [area for area in improvement_areas if area not in improvements]

            if remaining_areas:
                llm = ChatGroq(model="llama3-8b-8192", api_key=self.api_key, temperature=0.7)
                ...
                ai_improvements = {}
                try:
                    ai_improvements = json.loads(raw_content)
            ...
            # Ensure all requested areas have some response
            for area in improvement_areas:
    ...
    def get_improved_resume(self, target_role="", highlight_skills=""):
        ...
        try:
            ...
            # Parse highlight_skills parameter
            if highlight_skills:
                if len(highlight_skills) > 100:  # Assume it's a job description
            ...
            # Use analysis results if no specific skills provided
            ...
            # Prepare weakness context and improvement examples
            weakness_context = ""
            ...
            llm = ChatGroq(model="llama3-8b-8192", temperature=0.7, api_key=self.api_key)
            ...
            # Save improved resume to temporary file
            with tempfile.NamedTemporaryFile(delete=False, suffix='.txt', mode='w', encoding='utf-8') as tmp:
