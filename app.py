...
if __name__ == "__main__":
    try:
        ...
        st.error(f"🔥 Fatal Streamlit crash: {e}")
...
# st.title("🚀 Lightweight Resume Analyzer")

# api_key = st.text_input("Enter your Groq API Key", type="password")
# role = st.selectbox("Select Role", ["AI/ML Engineer", "Frontend Engineer", "Backend Engineer"])
# uploaded_file = st.file_uploader("Upload Resume", type=["pdf", "txt"])

# role_skills = {
#     "AI/ML Engineer": ["Python", "TensorFlow", "PyTorch", "Machine Learning"],
#     "Frontend Engineer": ["HTML", "CSS", "JavaScript", "React"],
#     "Backend Engineer": ["Python", "Django", "REST API", "Databases"]
# }

# if st.button("Analyze Resume"):
#     if not api_key or not uploaded_file:
#         st.warning("Please provide API key and resume file.")
#     else:
#         with st.spinner("Analyzing..."):
#             agent = ResumeAnalysisAgent(api_key=api_key)
#             result = agent.analyze_resume(uploaded_file, role_requirements=role_skills[role])
#             if result.get("error"):
#                 st.error(result["error"])
#             else:
#                 st.success(f"Score: {result['overall_score']}%")
#                 st.write("✅ Skills matched:", result["strengths"])
#                 st.write("❌ Skills missing:", result["missing_skills"])
